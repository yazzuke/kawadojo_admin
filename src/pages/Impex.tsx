...
import ModalItemImpex from '../components/ModalItemImpex';
...
const PartRow = ({ 
  ...
  const priceUsd = (part.price_yen || 0) / (Number(usdExchangeRate) || 160);
  ...
  return (
    <tr className="hover:bg-[#2a2a2a] transition-colors border-t border-gray-800">
      ...
      <td className="p-4 whitespace-nowrap text-sm text-blue-400 font-mono">US$ {priceUsd.toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</td>
      ...
      <td className="p-4 whitespace-nowrap text-center">
        <button 
          onClick={() => onSaveClick({
            ...part,
            kawadojo_price: sellPrice > 0 ? sellPrice : part.kawadojo_price,
            profit: profit !== 0 ? profit : part.profit,
            margin: margin !== 0 ? margin : part.margin
          })}
...
const SavedPartsTab = ({ 
  ...
  const handleDelete = async (partNoRaw: string) => {
    ...
    try {
      const res = await impexService.deletePart(partNoRaw);
      if (res.success) {
        toast.success(res.message || 'Pieza eliminada');
        fetchSavedParts();
      } else {
        toast.error(res.message || 'Error al eliminar la pieza');
      }
    } catch (err: any) {
      toast.error(err.message || 'Error al eliminar la pieza');
    }
  };
          ...
          <table className="w-full">
            <thead className="bg-[#1a1a1a] border-b border-gray-800">
              <tr className="text-left text-gray-400 text-xs">
                ...
                <th className="p-4 font-semibold uppercase tracking-wider">USD</th>
            ...
            <tbody className="bg-kawa-gray divide-y divide-gray-800">
              {savedParts.map(part => {
                const copCost = (part.price_yen || 0) * (Number(exchangeRate) || 0);
                const priceUsd = (part.price_yen || 0) / (Number(usdExchangeRate) || 160);
                const sellPrice = part.kawadojo_price || 0;
                const profit = sellPrice > 0 ? sellPrice - copCost : 0;
                const margin = copCost > 0 && sellPrice > 0 ? (profit / copCost) * 100 : 0;
                ...
                <tr key={part.id} className="hover:bg-[#2a2a2a] transition-colors">
                  ...
                  <td className="p-4 whitespace-nowrap text-sm font-semibold text-kawa-green">
                    $ {sellPrice.toLocaleString('es-CO', { maximumFractionDigits: 0 })}
                  </td>
                  <td className="p-4 whitespace-nowrap text-sm">
                    {sellPrice > 0 ? (
                      <div className="flex flex-col">
                  ...
                  <td className="p-4 whitespace-nowrap text-center">
                    <div className="flex items-center justify-center gap-2">
                      <button 
                        onClick={() => onEdit(part)}
                        className="bg-gray-800 text-white px-3 py-1 rounded-md hover:bg-gray-700 transition-colors text-xs font-bold"
                      >
                        Editar
                      </button>
                      <button 
                        onClick={() => handleDelete(part.part_no_raw)}
                        className="bg-red-900/50 text-red-400 px-3 py-1 rounded-md hover:bg-red-900 hover:text-red-300 transition-colors text-xs font-bold"
                      >
                        Eliminar
                      </button>
                    </div>
...
const Impex: React.FC = () => {
  const [activeTab, setActiveTab] = useState<'search' | 'saved' | 'quote'>('search');
  const [searchTerm, setSearchTerm] = useState('');
  const [exchangeRate, setExchangeRate] = useState<number | string>(21.23);
  const [usdExchangeRate, setUsdExchangeRate] = useState<number | string>(158.12);
  ...
  const handleOpenSaveModal = (part: any) => {
    setModalType('save');
    setFormData({
      mark: part.mark || '',
      part_no: part.part || '',
      ...
      name_en: part.name_en || part.name || '',
      price_yen: part.price_yen || 0,
      price_usd: part.price_usd || (part.price_yen ? Number((part.price_yen / (Number(usdExchangeRate) || 160)).toFixed(2)) : 0),
  ...
  const handleSaveSubmit = async () => {
    ...
    setIsModalOpen(false); // Close while saving to avoid double clicks
    try {
      if (modalType === 'save') {
        await impexService.savePart({
          mark: formData.mark,
          part_no: formData.part_no,
          part_no_raw: formData.part_no_raw,
          ...
          <div className="bg-kawa-gray border border-gray-800 rounded-xl p-6 mb-8 shadow-sm">
            <form onSubmit={handleSearch} className="flex flex-col lg:flex-row gap-6 items-end">
              ...
              <div className="w-full lg:w-auto flex gap-4">
                <div>
                  <label className="block text-sm font-medium text-gray-400 mb-2">Tasa (1 JPY = COP)</label>
                  <div className="flex items-center gap-2">
                    <span className="text-gray-500 font-medium">¥1 =</span>
                    <input
                      type="number"
                      step="any"
                      className="w-24 bg-[#1e1e1e] text-white border border-gray-700 rounded-lg py-2 px-3 focus:outline-none focus:border-kawa-green transition-colors font-mono"
                      value={exchangeRate}
                ...
                <div>
                  <label className="block text-sm font-medium text-gray-400 mb-2">Tasa (1 USD = JPY)</label>
                  <div className="flex items-center gap-2">
                    <span className="text-gray-500 font-medium">$1 =</span>
                    <input
                      type="number"
                ...
                {result.original_parts.length > 0 ? (
                  <div className="overflow-x-auto">
                    <table className="w-full">
                      ...
                      <tbody className="bg-kawa-gray divide-y divide-gray-800">
                        {result.original_parts.map(part => (
                          <PartRow 
                            key={part.part_no_raw} 
                            part={part} 
                ...
                {result.replacement_parts.length > 0 ? (
                  <div className="overflow-x-auto">
                    <table className="w-full">
                      ...
                      <tbody className="bg-kawa-gray divide-y divide-gray-800">
                        {result.replacement_parts.map(part => (
                          <PartRow 
                            key={part.part_no_raw} 
                            part={part} 
                            exchangeRate={exchangeRate} 
